...
export const HorizontalTimeline: React.FC<Props> = ({ data }) => {
  ...
  const currentReachedIndex = data.reduce((acc, _, i) => {
    const itemProgressStart = i / data.length;
    return progress > 0.05 + itemProgressStart * 0.9 ? i : acc;
  }, -1);
  ...
  return (
    <>
      <section
        ...
        style={isDesktop ? { height: `${totalScrollDistance + viewH}px` } : undefined}
      >
        {/* Mobile timeline */}
        <div className="md:hidden px-6 pt-16 pb-16">
          <div className="mb-10">
            <h2 className="text-[clamp(2.2rem,11vw,10rem)] font-black tracking-tighter leading-none text-black dark:text-white mb-3">
              私たちの歩み
            </h2>
          ...
          <div className="relative pl-6 border-l border-neutral-200 dark:border-neutral-800 space-y-8">
            {data.map((item, i) => (
              ...
              >
                <div className="absolute -left-[31px] top-1.5 w-3.5 h-3.5 rounded-full border-2 bg-white dark:bg-neutral-900 border-brand-blue shadow-[0_0_8px_rgba(0,107,182,0.35)]" />
                <span className="block text-sm font-bold tracking-widest uppercase text-brand-blue mb-1">
                  {item.year}
                </span>
                <h3 className="text-lg font-bold text-black dark:text-white mb-1.5 leading-snug">
                  {item.title}
                </h3>
                <p className="text-[15px] text-neutral-500 dark:text-neutral-400 leading-relaxed">
                  {item.summary}
                </p>
                <span className="inline-block mt-2 text-sm font-medium text-brand-blue opacity-80">
                  詳しく見る →
                </span>
              </div>
            ))}
        ...
        {/* Desktop timeline */}
        <div className="hidden md:flex sticky top-0 h-screen flex-col justify-center overflow-hidden relative">
          {/* Background image for active timeline point */}
          <AnimatePresence mode="sync">
            {currentReachedIndex >= 0 && data[currentReachedIndex]?.imageFile && (
              <motion.div
                key={`timeline-bg-${currentReachedIndex}`}
              ...
              >
                <img
                  src={`${publicUrl}/${data[currentReachedIndex].imageFile}`}
                  alt=""
                  className="absolute inset-0 w-full h-full object-cover"
                  style={{ clipPath: 'polygon(35% 0, 100% 0, 100% 100%, 0 65%)' }}
                />
                <div className="absolute inset-0 bg-gradient-to-tr from-white/95 via-white/70 to-white/20 dark:from-neutral-950/95 dark:via-neutral-950/70 dark:to-neutral-950/20" />
              </motion.div>
            )}
          </AnimatePresence>

          {/* Header */}
          <div className="relative z-10 pl-3 md:pl-6 lg:pl-10 mb-10">
            <h2 className="text-[clamp(2.2rem,11vw,10rem)] font-black tracking-tighter leading-none text-black dark:text-white mb-3">
              私たちの歩み
            </h2>
